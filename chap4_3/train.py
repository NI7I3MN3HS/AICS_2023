...
if __name__ == '__main__':
    ...
    image_style = image_style.to(torch.device('mlu'))
    net = VGG19().cpu()
    g_net = TransNet().cpu()
    g_net = g_net.to(torch.device('mlu'))
    net = net.to(torch.device('mlu'))
    ...
    loss_func = loss_func.to(torch.device('mlu'))
    ...
    while j <= epochs:
        for i, image in enumerate(data_loader):
            image_c = image.cpu()
            image_c = image_c.to(torch.device('mlu'))
            ...
            if i % 10 == 0:
                ...
                save_image(torch.cat((image_g, image_c), 3), f'./out/train/{count}.jpg')
        j += 1
